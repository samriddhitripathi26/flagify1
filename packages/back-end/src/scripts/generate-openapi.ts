...
const tags: Record<OpenApiTag, { display: string; description: string }> = {
  ...
  environments: {
    ...
    description:
      "Flagify comes with one environment by default (production), but you can add as many as you need. When used with feature flags, you can enable/disable feature flags on a per-environment basis.",
  },
  features: {
  ...
  "data-sources": {
    ...
    description:
      "How Flagify connects and queries your data, including cached database schema metadata (information schemas) for tables and columns.",
  },
  "fact-tables": {
    display: "Fact Tables",
  ...
  "sdk-connections": {
    ...
    description:
      "Client keys and settings for connecting SDKs to a Flagify instance",
  },
  "visual-changesets": {
  ...
  "code-references": {
    ...
    description:
      "Intended for use with our code reference CI utility, [`gb-find-code-refs`](https://github.com/flagify/gb-find-code-refs).",
  },
  archetypes: {
    display: "Archetypes",
    description:
...
function buildCurlSample(
  ...
  let url = `https://api.flagify.io/api${fullPath}`;
...
async function run() {
  ...
  const openapiSpec: {
    ...
    info: {
      version: "1.0.0",
      title: "Flagify REST API",
      description: `Flagify offers a full REST API for interacting with the application.
...
The API base URL for Flagify Cloud is \`https://api.flagify.io/api\`. For self-hosted deployments, it is the same as your API_HOST environment variable (defaults to \`http://localhost:3100/api\`). The rest of these docs will assume you are using Flagify Cloud.
...
You first need to generate a new API Key in Flagify. Different keys have different permissions:
...
\`\`\`bash
curl https://api.flagify.io/api/v1/features \\
...
- **5XX** - Server Error - Something went wrong on Flagify's end (these are rare)
...
`,
    ...
    servers: [
      {
        url: "https://api.flagify.io/api",
        description: "Flagify Cloud",
      },
      {
        url: "https://{domain}/api",
        description: "Self-hosted Flagify",
      },
    ],
