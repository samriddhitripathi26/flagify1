let Isolate: any;
let Reference: any;
let ExternalCopy: any;

try {
  const ivm = require("isolated-vm");
  Isolate = ivm.Isolate;
  Reference = ivm.Reference;
  ExternalCopy = ivm.ExternalCopy;
} catch (e) {
  // isolated-vm is not compiled/installed
}
...
export async function sandboxEval(
  ...
  if (IS_CLOUD) {
    return {
      ok: false,
      error: "Custom hooks are not supported in Flagify Cloud",
    };
  }
  ...
  if (!Isolate) {
    return {
      ok: false,
      error: "isolated-vm is not installed/compiled in this environment",
    };
  }
  ...
  try {
    const isolateCtx: any = await isolate.createContext();
    ...
    `;
    await isolate.compileScript(shimCode).then((s: any) => s.run(isolateCtx));
    ...
    const wrapped = `
      ...
      "__ready__";
    `;
    await isolate.compileScript(wrapped).then((s: any) => s.run(isolateCtx));
