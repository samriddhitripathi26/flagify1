...
export async function sendInviteEmail(
  ...
  await sendMail({
    html,
    subject: `You're invited to join ${noHyperlink(organization.name)} on Flagify`,
    to: invite.email,
    text: `${invite.invitedBy ? `${invite.invitedBy} is inviting you to` : "You've been invited to"} use Flagify with ${noHyperlink(organization.name)}. Accept your invitation: ${inviteUrl}`,
...
export async function sendResetPasswordEmail(email: string, resetUrl: string) {
  ...
  await sendMail({
    html,
    subject: "Reset Flagify Password",
    to: email,
...
export async function sendNewMemberEmail(
  ...
  await sendMail({
    html,
    subject: `A new user joined your Flagify account: ${noHyperlink(
      name,
    )} (${email})`,
    to: ownerEmail,
...
export async function sendPendingMemberEmail(
  ...
  await sendMail({
    html,
    subject: `A new user is requesting to join your Flagify account: ${noHyperlink(
      name,
    )} (${email})`,
    to: ownerEmail,
    text: `Organization: ${noHyperlink(organization)}\nName: ${noHyperlink(
...
export async function sendPendingMemberApprovalEmail(
  ...
  await sendMail({
    ...
    to: email,
    text: `Join ${noHyperlink(organization)} on Flagify`,
  });
}
...
export async function sendOwnerEmailChangeEmail(
  ...
  await sendMail({
    html,
    subject: `The owner for ${organization} on Flagify has changed`,
    to: originalOwner,
    text: `The owner for ${organization} on Flagify has been changed to ${newOwner} by ${email}`,
  });
