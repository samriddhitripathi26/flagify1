...
export async function createUser(
  ...
  const { externalId, displayName, userName, flagifyRole } = req.body;
  ...
  if (flagifyRole && isRoleValid(flagifyRole, org)) {
    // If a flagifyRole is provided, use that
    roleInfo = {
      role: flagifyRole,
  ...
  try {
    if (existingOrgMember && existingOrgMember.managedByIdp) {
      ...
      // If created through Flagify, update the user with the externalId and managedByIdp: true
    ...
    return res.status(400).json({
      schemas: ["urn:ietf:params:scim:api:messages:2.0:Error"],
      detail: `Unable to create the new user in Flagify: ${e.message}`,
      status: "400",
    });
  }
}
