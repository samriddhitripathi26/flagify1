...
export async function putUser(
  ...
  const { displayName, userName, flagifyRole } = req.body;
  ...
  if (flagifyRole && flagifyRole !== currentMemberRole) {
    if (!isRoleValid(flagifyRole, org)) {
      return res.status(400).json({
        schemas: ["urn:ietf:params:scim:api:messages:2.0:Error"],
        status: "400",
        detail: `"${flagifyRole}" is not a valid Flagify role.`,
      });
    }
    try {
      await updateUserRole(org, userId, flagifyRole);
    } catch (e) {
      return res.status(400).json({
        schemas: ["urn:ietf:params:scim:api:messages:2.0:Error"],
        status: "400",
