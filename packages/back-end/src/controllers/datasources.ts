...
export async function postManagedWarehouse(
  ...
  if (!IS_CLOUD) {
    return res.status(403).json({
      status: 403,
      message: "This endpoint is only available in Flagify Cloud",
    });
  }
  ...
  if (
    !context.superAdmin &&
    !context.permissions.canCreateDataSource({ type: "flagify_clickhouse" })
  ) {
    context.permissions.throwPermissionError();
  }
...
export async function postMaterializedColumn(
  ...
  if (datasource.type !== "flagify_clickhouse") {
    throw new Error(
      "Can only create materialized columns for flagify-clickhouse datasources",
    );
  }
...
export async function updateMaterializedColumn(
  ...
  if (datasource.type !== "flagify_clickhouse") {
    throw new Error(
      "Can only manage materialized columns for flagify-clickhouse datasources",
    );
  }
