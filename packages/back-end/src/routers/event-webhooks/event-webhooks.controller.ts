...
const testParamsPayload = (name: string) => ({
  text: `Hi there! This is a test event from Flagify to see if the params for webhook ${name} are correct.`,
  blocks: [
    {
      type: "section",
      text: {
        type: "mrkdwn",
        text: `*Hi there! 👋*\nThis is a *test event* from Flagify to see if the params for webhook ${name} are correct.`,
      },
    },
  ],
});
