...
describe("mongo utils", () => {
  describe("getConnectionStringWithDeprecatedKeysMigratedForV3to4", () => {
    it("returns the original string when no replacements are required", () => {
      const input =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin";
      const expected =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin";
    ...
    describe("when no connection options are provided", () => {
      it("returns the original string when no replacements are required", () => {
        const input = "mongodb://root:password@localhost:27017/flagify";
        const expected = "mongodb://root:password@localhost:27017/flagify";
    ...
    it("returns a modified URI when one of the properties needs to be migrated", () => {
      const input =
        "mongodb://root:password@localhost:27017/flagify?poolSize=50";
      const expected =
        "mongodb://root:password@localhost:27017/flagify?maxPoolSize=50";
    ...
    it("returns a modified URI when several properties need to be migrated", () => {
      const input =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin&poolSize=50&wtimeout=123456&appname=foobar&tlsinsecure=true";
      const expected =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin&maxPoolSize=50&tlsInsecure=true&wtimeoutMS=123456&appName=foobar";
    ...
    it("reports unsupported keys when known v3 options without adequate v4 mappings are provided, leaving invalid keys in the connection string", () => {
      const input =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin&poolSize=50&wtimeout=123456&appname=foobar&tlsinsecure=true&autoReconnect=true&reconnectRetries=3&reconnectInterval=300&ha=true&haInterval=300&secondaryAcceptableLatencyMS=1000&acceptableLatencyMS=1000&j=true&connectWithNoPrimary=true&domainsEnabled=false&bufferMaxEntries=10&foo=1&bar=2&baz=3";
      // includes original deprecated keys
      const expected =
        "mongodb://root:password@localhost:27017/flagify?authSource=admin&autoReconnect=true&reconnectRetries=3&reconnectInterval=300&ha=true&haInterval=300&secondaryAcceptableLatencyMS=1000&acceptableLatencyMS=1000&connectWithNoPrimary=true&domainsEnabled=false&bufferMaxEntries=10&foo=1&bar=2&baz=3&maxPoolSize=50&tlsInsecure=true&wtimeoutMS=123456&journal=true&appName=foobar";
  ...
  describe("isDuplicateKeyError", () => {
    ...
    it("returns true when the message contains 'E11000' as a last-resort fallback", () => {
      expect(
        isDuplicateKeyError({
          message:
            'E11000 duplicate key error collection: flagify.featurerevisions index: organization_1_featureId_1_version_1 dup key: { organization: "org_1", featureId: "f", version: 369 }',
        }),
      ).toBe(true);
    });
    ...
    it("matches real Error instances that carry the E11000 message", () => {
      ...
      const err = new Error(
        "E11000 duplicate key error collection: flagify.featurerevisions",
      );
