...
This document is meant for developers who want to contribute to the Flagify platform. It covers the following topics:
...
Users in Flagify have roles which grant them permissions to do various actions. Any endpoint which creates, edits, or deletes should check permissions as part of request handler.
...
Flagify follows an "Open Core" model, where the majority of the application is free and open source, but there are commercial features added on top that require a paid license.

It's also sometimes necessary to differentiate between self-hosted deployments and Flagify Cloud. We try to keep these checks to a minimum in the code to make the platform easier to test and maintain.
...
And an environment variable to check if you are on Flagify Cloud:
...
### Testing Flagify Cloud functionality locally
...
Note: The above only works when APP_ORIGIN is localhost (or it's running on the real Flagify Cloud).
...
To use the analysis parts of Flagify, you need to connect to a data source with experimentation events.
...
You can do this from your terminal with the following command, if your local postgres db is running with user `postgres`, database `flagify_db`,

```bash
psql -U postgres -d flagify_db -a -f packages/back-end/test/data-generator/sql_scripts/create_postgres.sql
```
...
Then, you can run `packages/back-end/test/data-generator/sql_scripts/create_mysql.sql` from the mysql console or from the terminal like so (using example user `myuser` and example database `flagify_db`):

```bash
mysql -u myuser -p flagify_db --local-infile < packages/back-end/test/data-generator/sql_scripts/create_mysql.sql
```

**Snowflake**

Create a database and a schema in your Snowflake. Then, you can run `packages/back-end/test/data-generator/sql_scripts/create_snowflake.sql` using the `snowsql` command line client (see their [docs](https://docs.snowflake.com/en/user-guide/snowsql.html)). For example, you can run the following in your Mac or Linux terminal if you have a database called `flagify_db`, a schema called `sample`, account name called `account-name`, and you want to use user `myuser` to log in.

```bash
snowsql -d flagify_db -s sample -a account-name -u myuser -f packages/back-end/test/data-generator/sql_scripts/create_snowflake.sql
```
...
Next, you'll need to set up the connection to your DB from within Flagify.
