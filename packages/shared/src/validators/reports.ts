...
export const reportShareLevelSchema = z
  ...
  .describe(
    "Visibility of the report. `private` (default) restricts access to the API caller and admins. `organization` makes it visible to all members of the organization in the Flagify UI. `public` additionally exposes it via a shareable URL (returned as `shareUrl`); anyone with the URL can view it without authentication.",
  );
...
const putReportMetadataBody = z
  .object({
    ...
    editLevel: z
      ...
      .describe(
        "Who can edit the report in the Flagify UI. `organization` allows any org member with the `createAnalyses` permission; `private` restricts editing to the report owner.",
      )
      .optional(),
  })
  .strict();
