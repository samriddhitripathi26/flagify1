...
[Flagify](https://www.flagify.io) is a modular Feature Flagging and Experimentation platform.

This is the React client library that lets you evaluate feature flags and run experiments (A/B tests) within a React application. It is a thin wrapper around the [Javascript SDK](https://docs.flagify.io/lib/js), so you might want to view those docs first to familiarize yourself with the basic classes and methods.

![Build Status](https://github.com/flagify/flagify/workflows/CI/badge.svg) ![GZIP Size](https://img.shields.io/badge/gzip%20size-7.84KB-informational) ![NPM Version](https://img.shields.io/npm/v/@flagify/flagify-react)
...
- Run **Visual Experiments** without writing code by using the Flagify Visual Editor

## Community

Join [our Flagify Users Slack community](https://slack.flagify.io?ref=react-readme) if you need help, want to chat, or are thinking of a new feature. We're here to help - and to make Flagify even better.

## Installation

```
yarn add @flagify/flagify-react
```

or

```
npm install --save @flagify/flagify-react
```
...
import { Flagify, FlagifyProvider } from "@flagify/flagify-react";

// Create a Flagify instance
const gb = new Flagify({
  apiHost: "https://cdn.flagify.io",
  clientKey: "sdk-abc123",
  // Enable easier debugging during development
  enableDevMode: true,
  // Update the instance in realtime as features change in Flagify
...
There are a few ways to use feature flags in Flagify:

#### Feature Hooks

```tsx
import { useFeatureValue, useFeatureIsOn } from "@flagify/flagify-react";
...
```tsx
import { IfFeatureEnabled, FeatureString } from "@flagify/flagify-react";
...
If you need low-level access to the Flagify instance for any reason, you can use the `useFlagify` hook.
...
In order for the Flagify SDK to work, it needs to have feature definitions from the Flagify API. There are 2 ways to get this data into the SDK.

### Built-in Fetching and Caching

If you pass an `apiHost` and `clientKey` into the Flagify constructor, it will handle the network requests, caching, retry logic, etc. for you automatically. If your feature payload is encrypted, you can also pass in a `decryptionKey`.

```ts
const gb = new Flagify({
  apiHost: "https://cdn.flagify.io",
  clientKey: "sdk-abc123",
  // Only required if you have feature encryption enabled in Flagify
  decryptionKey: "key_abc123",
  // Update the instance in realtime as features change in Flagify (default: false)
  subscribeToChanges: true,
});
...
By default, the SDK will open a streaming connection using Server-Sent Events (SSE) to receive feature updates in realtime as they are changed in Flagify. This is only supported on Flagify Cloud or if running a Flagify Proxy Server.

If you want to disable streaming updates (to limit API usage on Flagify Cloud for example), you can set `backgroundSync` to `false`.
...
Note that you don't have to call `gb.init()` or `gb.loadFeatures()`. There's nothing to load - everything required is already passed in. No network requests are made to Flagify at all.
...
If you want more control, you can use the `useFlagify()` hook and the `ready` flag:
...
Experiments created through the Flagify Visual Editor will run automatically as soon as their targeting conditions are met.
...
If you are using this SDK in a Single Page App (SPA), you will need to let the Flagify instance know when the URL changes so the active experiments can update accordingly.

For example, in Next.js, you could do this:

```js
function updateFlagifyURL() {
  gb.setURL(window.location.href);
}

export default function MyApp() {
  // Subscribe to route change events and update Flagify
  const router = useRouter();
  useEffect(() => {
    router.events.on("routeChangeComplete", updateFlagifyURL);
    return () => router.events.off("routeChangeComplete", updateFlagifyURL);
  }, []);
...
There are 2 ways to use Flagify for SSR.
...
export async function getServerSideProps(context) {
  // Create a Flagify instance and load features from the API
  const gb = new Flagify({
    apiHost: "https://cdn.flagify.io",
    clientKey: "sdk-abc123",
    attributes: {
...
First, create a helper function to generate Flagify SSR props from an incoming request

```js
// util/gb-server.js
import { getFlagifySSRData } from "@flagify/flagify-react";

export async function generateFlagifySSRProps(context) {
  return getFlagifySSRData({
    apiHost: process.env.NEXT_PUBLIC_FLAGIFY_API_HOST,
    clientKey: process.env.NEXT_PUBLIC_FLAGIFY_CLIENT_KEY,
    attributes: {
      // TODO: get more targeting attributes from request context
      id: context.req.cookies.DEVICE_ID,
    },
...
Then, follow the same steps as client-side rendering to wrap your app in a FlagifyProvider:
...
// Create a client-side Flagify instance
const gb = new Flagify({
  apiHost: "https://cdn.flagify.io",
  clientKey: "sdk-abc123",
...
Now, on a server rendered page, call the helper you made and pass it into the `useFlagifySSR` hook:

```jsx
// pages/server.jsx
import MyComponent from "../components/MyComponent";
import { generateFlagifySSRProps } from "../util/gb-server";
import { useFlagifySSR } from "@flagify/flagify-react";

export async function getServerSideProps(context) {
  const gbData = await generateFlagifySSRProps(context);
  return {
    props: {
      gbData,
    },
...
There are a number of configuration options and settings that control how Flagify behaves.
...
Flagify can fire a callback whenever a feature is evaluated for a user. This can be useful to update 3rd party tools like NewRelic or DataDog.
...
There is a [Flagify Chrome DevTools Extension](https://chrome.google.com/webstore/detail/flagify-devtools/opemhndcehfgipokneipaafbglcecjia) that can help you debug and test your feature flags in development.

In order for this to work, you must explicitly enable dev mode when creating your Flagify instance:
...
View the [Javascript SDK Docs](https://docs.flagify.io/lib/js) for all of the options available for inline experiments

#### useExperiment hook

```tsx
import { useExperiment } from "@flagify/flagify-react";
...
```tsx
import { withRunExperiment } from "@flagify/flagify-react";
...
- [React Native <ExternalLink />](https://github.com/flagify/examples/tree/main/react-native-cli)
