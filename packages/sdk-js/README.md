# Flagify Javascript SDK

[Flagify](https://www.flagify.io) is an open source Feature Flagging and Experimentation platform.
...
![Build Status](https://github.com/flagify/flagify/workflows/CI/badge.svg) ![GZIP Size](https://img.shields.io/badge/gzip%20size-6.9KB-informational) ![NPM Version](https://img.shields.io/npm/v/@flagify/flagify)
...
```html
<!-- Creates `window.flagify` with all of the exported classes -->
<script src="https://cdn.jsdelivr.net/npm/@flagify/flagify/dist/bundles/index.js"></script>
```
...
```js
const { setPolyfills } = require("@flagify/flagify");
...
There are 2 ways to use Flagify in a Node.js environment depending on where you want to declare user attributes.

### Instance per Request

In this mode, you create a separate Flagify instance for every incoming request that includes user-specific attributes. This has more overhead, but ensures you only need to define user attributes once instead of passing them throughout your app.
...
app.use(function (req, res, next) {
  // Create a Flagify instance and store in the request
  req.flagify = new Flagify({
    apiHost: "https://cdn.flagify.io",
    clientKey: "sdk-abc123",
  ...
  res.on("close", () => req.flagify.destroy());
  ...
  req.flagify
    ...
    .then(() => next())
    .catch((e) => {
      console.error("Failed to load features from Flagify", e);
      next();
    });
});
```

Then, you can access the Flagify instance from any route:

```js
app.get("/", (req, res) => {
  const gb = req.flagify;
  // ...
});
```
...
In this mode, you create a singleton FlagifyClient instance that is shared between all requests and you pass in user attributes when calling feature methods like `isOn`.

This method is more efficient, but does require you to pass around a `userContext` throughout your code.

```js
// Create a multi-user Flagify instance without user-specific attributes
const gb = new FlagifyClient({
  apiHost: "https://cdn.flagify.io",
  clientKey: "sdk-abc123",
});
await gb.init();
...
When used in a TypeScript project, Flagify includes basic type inference out of the box:
...
If you want to enforce stricter types in your application, you can do that when creating the Flagify instance:
...
// Pass into the Flagify instance
const gb = new Flagify<AppFeatures>({
  ...
});
```
...
Instead of defining the `AppFeatures` interface manually like above, you can auto-generate it from your Flagify account using the [Flagify CLI](https://docs.flagify.io/tools/cli).

## Flagify Instance (reference)
...
You can also define targeting conditions that limit which users are included in the experiment. These conditions are evaluated against the `attributes` passed into the Flagify context. The syntax for conditions is based on the MongoDB query syntax and is straightforward to read and write.
...
Rules can optionally define targeting conditions that limit which users the rule applies to. These conditions are evaluated against the `attributes` passed into the Flagify context. The syntax for conditions is based on the MongoDB query syntax and is straightforward to read and write.
...
- [Typescript example app with strict typing <ExternalLink />](https://github.com/flagify/examples/tree/main/vanilla-typescript).
