...
export function autoAttributesPlugin(settings: AutoAttributeSettings = {}) {
  ...
  document.addEventListener("flagifypersist", () => {
    persistUUID();
  });
  ...
  return (gb: Flagify | UserScopedFlagify | FlagifyClient) => {
    ...
    // Poll for URL changes and update Flagify
    ...
    document.addEventListener("flagifyrefresh", refreshListener);

    if ("onDestroy" in gb) {
      gb.onDestroy(() => {
        clearInterval(intervalTimer);
        document.removeEventListener("flagifyrefresh", refreshListener);
      });
    }
  };
}
