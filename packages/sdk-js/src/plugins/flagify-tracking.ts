...
import type { Attributes, EventProperties } from "../types/flagify";
import type { Flagify } from "../Flagify";
import type {
  FlagifyClient,
  UserScopedFlagify,
} from "../FlagifyClient";
...
export function flagifyTrackingPlugin({
  ...
  return (gb: Flagify | UserScopedFlagify | FlagifyClient) => {
    ...
    if ("setEventLogger" in gb) {
      ...
      // Flush the queue when the flagify instance is destroyed
      "onDestroy" in gb &&
        gb.onDestroy(() => {
          flush().catch(console.error);
        });
      ...
      window.gbEvents = {
        push: (event: GlobalTrackedEvent | string) => {
          if ("isDestroyed" in gb && gb.isDestroyed()) {
            ...
            // This will let the next Flagify instance pick it up
