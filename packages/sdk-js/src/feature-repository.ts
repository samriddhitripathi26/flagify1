...
export async function refreshFeatures({
  ...
  instance: Flagify | FlagifyClient;
  timeout?: number;
  skipCache?: boolean;
  allowStale?: boolean;
...
// Subscribe a Flagify instance to feature changes
function subscribe(instance: Flagify | FlagifyClient): void {
  const key = getKey(instance);
  const subs = subscribedInstances.get(key) || new Set();
  subs.add(instance);
  subscribedInstances.set(key, subs);
}
export function unsubscribe(instance: Flagify | FlagifyClient): void {
...
function getKey(instance: Flagify | FlagifyClient): string {
  const [apiHost, clientKey] = instance.getApiInfo();
  return `${apiHost}||${clientKey}`;
}

function getCacheKey(instance: Flagify | FlagifyClient): string {
...
function onNewFeatureData(
  ...
  // Update features for all subscribed Flagify instances
...
export function clearAutoRefresh() {
  ...
  // Remove all references to Flagify instances
