...
> {
  ...
  constructor(options?: Options) {
    ...
    if (options.remoteEval) {
      ...
      if (isGbHost) {
        throw new Error("Cannot use remoteEval on Flagify Cloud");
      }
    } else {
  ...
  public initSync(options: InitSyncOptions): Flagify {
  ...
  public destroy(options?: DestroyOptions) {
    ...
    if (isBrowser && window._flagify === this) {
      delete window._flagify;
    }
  ...
  public async logEvent(
    ...
    if (this._destroyed) {
      console.error("Cannot log event to destroyed Flagify instance");
      return;
    }
...
export async function prefetchPayload(options: PrefetchOptions) {
  ...
  const instance = new Flagify(options);
