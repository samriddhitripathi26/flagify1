...
backendPackageJson.dependencies["@flagify/flagify"] = `^${newVersion}`;
fs.writeFileSync(
...
sharedPackageJson.dependencies["@flagify/flagify"] = `^${newVersion}`;
fs.writeFileSync(
...
sdkReactPackageJson.dependencies["@flagify/flagify"] = `^${newVersion}`;
fs.writeFileSync(
...
frontendPackageJson.dependencies["@flagify/flagify-react"] =
  `^${newVersion}`;
fs.writeFileSync(
...
topLevelPackageJson.pnpm.overrides["@flagify/flagify"] = newVersion;
fs.writeFileSync(
  path.resolve(__dirname, "../../../package.json"),
