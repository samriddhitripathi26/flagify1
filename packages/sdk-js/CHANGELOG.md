...
- Fix incorrect `version` property on Flagify instances (was still set to 1.5.1)
...
- New `devtools` plugin to integrate back-end code with the Flagify Dev Tools browser extension
...
- In `auto.min.js`, enable dev mode by default. Without this, the Flagify DevTools Chrome extension will only partially work.

## **1.4.0** - Feb 19, 2025

- Fixed edge case with pre-requisites that caused some feature rules to be skipped
- New methods for dealing with destroyed Flagify instances - `isDestroyed()` and `onDestroy(callback)`
- New `plugins` option to extend Flagify functionality, plus several built-in plugins
...
- Renamed `FlagifyMultiUser` to `FlagifyClient`
- New `FlagifyClient.createScopedInstance()` method to make it easier to use in express-like back-end frameworks and client-side environments.

## **1.3.0** - Nov 20, 2024

- New `FlagifyMultiUser` class for 2x performance boost in Node.js
- Remove undocumented and deprecated `Flagify` constructor options: `realtimeKey`, `realtimeInterval`, and `stickyBucketIdentifierAttributes`.
...
- Fix bug when passing in `stickyBucketAssignmentDocs` to the Flagify constructor
...
- Disable in-memory cache via `configureCache({ disableCache: true })` or the new Flagify constructor option `disableCache`.
- Easier streaming customization via `init({ streaming: true })`.
- Prefetch payloads (and optionally begin streaming) before you create a Flagify instance with `prefetchPayload()`.
...
- Network requests are no longer started when creating a Flagify instance. It now waits until `loadFeatures` (or the new `init`) is called.
...
- New methods to deal with deferred tracking calls and server/client hydration
  ...
  - `setDeferredTrackingCalls` - Hydrate the Flagify instance with queued tracking calls (in JSON format). Does not fire these calls immediately.
...
- Various improvements to `auto.min.js` script
  ...
  - New `data-no-auto-cookies` script attribute and `flagifypersist` event listener to more easily work with cookie consent banners
  - New `flagifydata` DOM event to attach multiple renderers to a single Flagify instance
...
- Various improvements to `auto.min.js` script
  - New `window.flagify_queue` to make it easier to work with an async script tag
  - New `pageTitle` auto attribute
  - Fixed bugs with built-in GA4 and GTM tracking calls
  - New `flagifyrefresh` DOM event you can trigger to for re-evaluation of auto attributes
...
- Sticky Bucketing support with built-in implementations for persisting in localStorage, cookies (both browser and Node.js), and Redis. Off by default, an implementation must be passed into the Flagify constructor to enable.
...
- Fix bug when using the Flagify DevTools Extension with Visual Editor experiments
...
- New context option `backgroundSync`. Set to false (default true) to disable opening a streaming connection to Flagify in the background
- New context option `subscribeToChanges`. Set to true (default false) to update the instance in realtime as features change in Flagify. This replaces the old `autoRefresh` setting (the old setting continues to work for backwards compatibility)
...
- Update Flagify test suite to v0.4.2
...
- Support for the new Flagify Visual Editor
- Advanced URL targeting support

## **0.23.0** - Feb 27, 2023

- Update Flagify test suite to v0.4.0
...
- Support new Flagify DevTools Chrome extension
