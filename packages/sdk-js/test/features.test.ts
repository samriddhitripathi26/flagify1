import { Context, Experiment, Flagify } from "../src";
...
describe("features", () => {
  ...
  it("decrypts features with custom SubtleCrypto implementation", async () => {
    ...
    await flagify.setEncryptedFeatures(
      encryptedFeatures,
      keyString,
      webcrypto.subtle,
    );
  ...
  it("decrypts features using the native SubtleCrypto implementation", async () => {
    ...
    await flagify.setEncryptedFeatures(encryptedFeatures, keyString);
  ...
  it("throws when decrypting features and no SubtleCrypto implementation exists", async () => {
    ...
    await expect(
      flagify.setEncryptedFeatures(encryptedFeatures, keyString),
  ...
  it("can set features asynchronously", () => {
    ...
    expect(flagify.feature("feature")).toEqual({
      value: 0,
      on: false,
      off: true,
      ruleId: "",
  ...
  it("returns ruleId when evaluating a feature", () => {
    ...
    expect(flagify.evalFeature("feature").ruleId).toEqual("foo");
    flagify.destroy();
  });
  ...
  it("updates attributes with setAttributes", () => {
    ...
    flagify.setAttributes({ foo: 2, baz: 3 });
  ...
  it("uses attribute overrides", () => {
    ...
    flagify.setAttributeOverrides({
      foo: "baz",
    });

    expect(flagify.getAttributes()).toEqual({
      id: "123",
      foo: "baz",
    });
    expect(
  ...
  it("uses forced feature values", () => {
    ...
    expect(flagify.feature("feature1").value).toEqual(0);
    expect(flagify.feature("feature2").value).toEqual(1);
    expect(flagify.feature("feature3").value).toEqual(1);

    flagify.setForcedFeatures(new Map());
    expect(flagify.feature("feature1").value).toEqual(0);
    expect(flagify.feature("feature2").value).toEqual(0);
    expect(flagify.feature("feature3").value).toEqual(null);
  ...
  it("gets features", () => {
    ...
    expect(flagify.getFeatures()).toEqual(features);
  ...
  it("fires feature usage callback", () => {
    ...
    flagify.setForcedFeatures(forcedFeatures);
    const mock = mockCallback(context);

    // Fires for regular features
    const res1 = flagify.evalFeature("feature1");
    // Fires for unknown features
    const res2 = flagify.evalFeature("feature2");
    // Does not fire for repeats
    flagify.evalFeature("feature1");
    // Does not fire when value is forced via an override
    flagify.evalFeature("feature3");
  ...
  it("uses fallbacks get getFeatureValue", () => {
    ...
    expect(flagify.getFeatureValue("feature", "green")).toEqual("blue");
    expect(flagify.getFeatureValue("unknown", "green")).toEqual("green");
    expect(flagify.getFeatureValue("testing", null)).toEqual(null);
  ...
  it("fires remote tracking calls", async () => {
    ...
    const res = flagify.evalFeature("feature");
  ...
  it("gates flag rule evaluation on prerequisite flag", async () => {
    ...
    const missingResult = flagify.evalFeature("childFlagWithMissingPrereq");
    expect(missingResult.value).toEqual(null);

    const result1 = flagify.evalFeature("childFlag");
    ...
    const result2 = flagify.evalFeature("childFlag");
  ...
  it("gates experiment rule evaluation on prerequisite flag", async () => {
    ...
    const exp = flagify.getExperiments()[0];
    const result1 = flagify.run(exp);
  ...
  it("conditionally applies a force rule based on prerequisite targeting", async () => {
    ...
    const result3 = flagify.evalFeature("childFlag");
  ...
  it("conditionally applies a force rule based on prerequisite JSON targeting", async () => {
    ...
    const result1a = flagify.evalFeature("childFlag");
    expect(result1a.value).toEqual("dark mode");

    const result1b = flagify.evalFeature("childFlag2");
  ...
  it("returns null when hitting a prerequisite cycle", async () => {
    ...
    const result = flagify.evalFeature("childFlag");
