...
describe("feature-repo", () => {
  ...
  it("debounces fetch requests", async () => {
    ...
    expect(flagify1.evalFeature("foo").value).toEqual("initial");
    expect(flagify2.evalFeature("foo").value).toEqual("initial");
    expect(flagify3.evalFeature("foo").value).toEqual("initial");
  ...
  it("prefetches", async () => {
    ...
    expect(flagify.getPayload()).toEqual(apiPayload);
  ...
  it("uses cache and can refresh manually", async () => {
    ...
    expect(flagify2.evalFeature("foo").value).toEqual(null);
    ...
    expect(flagify3.evalFeature("foo").value).toEqual(null);
    await flagify3.loadFeatures({ autoRefresh: true });
    expect(flagify3.evalFeature("foo").value).toEqual("initial");
    ...
    expect(flagify4.evalFeature("foo").value).toEqual(null);
    await flagify4.loadFeatures();
    expect(flagify4.evalFeature("foo").value).toEqual("initial");
    ...
    expect(flagify.evalFeature("foo").value).toEqual("changed");

    // The instance with `subscribeToChanges` should now have the new value
    expect(flagify2.evalFeature("foo").value).toEqual("changed");

    // The instance with `autoRefresh` should now have the new value
    expect(flagify3.evalFeature("foo").value).toEqual("changed");
    ...
    expect(flagify5.evalFeature("foo").value).toEqual(null);
    await flagify5.loadFeatures();
    expect(flagify5.evalFeature("foo").value).toEqual("changed");
  ...
  it("uses localStorage cache", async () => {
    ...
    await flagify.loadFeatures({ autoRefresh: true });
  ...
  it("prefetches with SSE", async () => {
    ...
    await flagify2.init({ streaming: true });
    expect(f.mock.calls.length).toEqual(1);
    expect(flagify2.evalFeature("foo").value).toEqual("api");
    expect(flagify2.getPayload()).toEqual(apiPayload);
    ...
    expect(flagify2.evalFeature("foo").value).toEqual("streaming");
    expect(flagify2.getPayload()).toEqual(streamingPayload);
    ...
    await flagify3.init();
    expect(flagify3.evalFeature("foo").value).toEqual("streaming");
  ...
  it("Can use streaming with init({payload})", async () => {
    ...
    await flagify.init({ payload: hydratedPayload, streaming: true });
    ...
    expect(flagify.evalFeature("foo").value).toEqual("streaming");
  ...
  it("updates features based on SSE", async () => {
    ...
    await Promise.all([
      ...
      flagify2.loadFeatures({ autoRefresh: true }),
    ]);
  ...
  it("exposes a gb.ready flag", async () => {
    ...
    expect(flagify.ready).toEqual(false);
    await flagify.loadFeatures();
    expect(f.mock.calls.length).toEqual(1);
    expect(flagify.ready).toEqual(true);
  ...
  it("handles broken fetch responses gracefully", async () => {
    ...
    expect(flagify.ready).toEqual(true);
    expect(flagify.getFeatures()).toEqual({});
  ...
  it("handles super long API requests gracefully", async () => {
    ...
    await flagify.loadFeatures({ timeout: 20 });
    expect(f.mock.calls.length).toEqual(1);
    // Ready state changes to true
    expect(flagify.ready).toEqual(true);
    expect(flagify.getFeatures()).toEqual({});
    ...
    await sleep(100);
    await flagify.refreshFeatures({ timeout: 20 });
    expect(f.mock.calls.length).toEqual(1);
    expect(flagify.ready).toEqual(true);
    expect(flagify.getFeatures()).toEqual({
      foo: {
  ...
  it("handles timeouts with init", async () => {
    ...
    expect(flagify.ready).toEqual(false);
    // Doesn't throw errors
    const res = await flagify.init({ timeout: 20 });
    expect(f.mock.calls.length).toEqual(1);
    expect(flagify.ready).toEqual(true);
    expect(flagify.getFeatures()).toEqual({});
    ...
    const res2 = await flagify2.init({ timeout: 20 });
    expect(res2.success).toEqual(true);
    expect(flagify2.getFeatures()).toEqual(payload.features);
    ...
    const res3 = await flagify3.init({ timeout: 200 });
    expect(res3.success).toEqual(true);
    expect(flagify3.getFeatures()).toEqual(payload.features);
  ...
  it("handles errors with init", async () => {
    ...
    await flagify.setPayload(payload);

    // Refreshing with an error, logs the error, but doesn't overwrite the payload
    await flagify.refreshFeatures();
    expect(flagify.getFeatures()).toEqual(payload.features);
    expect(f.mock.calls.length).toEqual(2);
    expect(log.mock.calls.length).toEqual(2);
    expect(log.mock.calls[1][0]).toEqual("Error fetching features");
  ...
  it("loads features from a hydrated payload", async () => {
    ...
    expect(flagify.evalFeature("foo").value).toEqual("hydrated");
    expect(f.mock.calls.length).toEqual(0);
    ...
    await flagify.setPayload({ features: hydratedFeatures });
    expect(flagify.evalFeature("foo").value).toEqual("new hydrated value");
    expect(f.mock.calls.length).toEqual(1);
  ...
  it("preserves both an encrypted and unencrypted payload", async () => {
    ...
    // Initial value of feature should be null
    expect(flagify.evalFeature("testfeature1").value).toEqual(null);
    ...
    expect(flagify.evalFeature("testfeature1").value).toEqual(true);

    expect(flagify.getPayload()).toEqual({
      encryptedFeatures:
        "vMSg2Bj/IurObDsWVmvkUg==.L6qtQkIzKDoE2Dix6IAKDcVel8PHUnzJ7JjmLjFZFQDqidRIoCxKmvxvUj2kTuHFTQ3/NJ3D6XhxhXXv2+dsXpw5woQf0eAgqrcxHrbtFORs18tRXRZza7zqgzwvcznx",
    });

    expect(flagify.getDecryptedPayload()).toEqual({
      features: {
        testfeature1: {
          defaultValue: true,
          rules: [
  ...
  it("can disableCache", async () => {
    ...
    // Each new Flagify instance hits the API
    const flagify = new Flagify({
      apiHost: "https://fakeapi.sample.io",
      clientKey: "sdk-abc123",
    });
