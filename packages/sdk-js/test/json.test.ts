...
describe("json test suite", () => {
  it.each((cases as Cases).feature)(
    ...
    (name, ctx, key, expected) => {
      const flagify = new Flagify(ctx);
      expect(flagify.evalFeature(key)).toEqual(expected);
      flagify.destroy();
    },
  );
  ...
  it.each((cases as Cases).stickyBucket)(
    ...
    async (
      ...
      await sleep(10);
      expect(flagify.evalFeature(key).experimentResult ?? null).toEqual(
        expectedExperimentResult,
      );
      expect(flagify.getStickyBucketAssignmentDocs()).toEqual(
        expectedStickyBucketAssignmentDocs,
      );
      flagify.destroy();
    },
  ...
  it.each((cases as Cases).urlRedirect)(
    ...
    async (name, ctx, result) => {
      const flagify = new Flagify(ctx);
      await sleep();
      const trackingCalls = flagify.getDeferredTrackingCalls();
      const actualResult: {
        ...
        urlWithParams: flagify.getRedirectUrl(),
      }));
