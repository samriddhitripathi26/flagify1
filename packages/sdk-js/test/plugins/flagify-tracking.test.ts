import { flagifyTrackingPlugin } from "../../src/plugins/flagify-tracking";
import {
...
describe("flagifyTrackingPlugin", () => {
  ...
  it("Logs events to the server", async () => {
    const plugin = flagifyTrackingPlugin();
  ...
  it("Can be disabled and debugged", async () => {
    ...
    expect(log).toHaveBeenCalledWith("Logging event to Flagify", {
  ...
  it("Picks up events logged to window.gbEvents", async () => {
    ...
    // A new Flagify instance does not pick up the events that have already been fired
  ...
  it("works for FlagifyClient and user-scoped instances", async () => {
    const plugin = flagifyTrackingPlugin({
