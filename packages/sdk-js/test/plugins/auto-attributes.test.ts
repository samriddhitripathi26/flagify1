import { Flagify, FlagifyClient } from "../../src";
...
describe("autoAttributesPlugin", () => {
  ...
  it("parses UTM parameters from querystring and stores in session storage", () => {
    ...
    // UTM should still be picked up on a new Flagify instance with a different URL
    setWindowURL("http://localhost/");
  ...
  it("can disable uuid auto-persist", () => {
    ...
    // Trigger persist
    document.dispatchEvent(new Event("flagifypersist"));
    expect(getCookie("gbuuid")).toBe(uuid);

    gb.destroy();
  });

  it("works with FlagifyClient and user-scoped instances", () => {
    const plugin = autoAttributesPlugin();
