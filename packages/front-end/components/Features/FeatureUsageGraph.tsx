...
export function FeatureUsageProvider({
  ...
  const flagifyManagedDatasource = datasources.find(
    (ds) => ds.type === "flagify_clickhouse",
  );
  const managedWarehouseAwaitingProvisioning = flagifyManagedDatasource
    ? isManagedWarehouseAwaitingProvisioning(flagifyManagedDatasource)
    : false;
  const showFeatureUsage = useDummyData || !!flagifyManagedDatasource;
  ...
  const featureUsageAutoRefreshInterval = flagify.getFeatureValue(
