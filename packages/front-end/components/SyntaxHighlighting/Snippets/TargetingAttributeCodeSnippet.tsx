    ...
    if (usePlugins) {
      ...
      if (additionalAttributes.length) {
        attributesSnippets.push(
          <>
            ...
            <Code
              language="javascript"
              code={`flagify.updateAttributes(${stringify(
      ...
      attributesSnippets.push(
        <>
          <Code
            language="javascript"
            code={`flagify.setAttributes(${stringify(exampleAttributes)});`}
          />
        </>,
      );
    }
    ...
    if (eventTracker === "mixpanel") {
      attributesSnippets.push(
        <Box>
          ...
          to pass this id to Flagify. This might need to be adjusted to wait
          for Mixpanel to load.
          <Code
            language="javascript"
            code={`
// Add the mixpanel user id to the Flagify attributes when it loads:
    ...
    domain_user_id: this.sp.getDomainUserId(),
      ...
      attributesSnippets.push(
        <Box>
          ...
          (recommended) you need to pass in this ID to Flagify.
          <Code
            language="javascript"
    ...
    function () {
      ...
      attributesSnippets.push(
        <Box>
          ...
          (recommended), the ID needs to be passed to Flagify. This might
          need to be adjusted to wait for Amplitude to load.
          <Code
            language="javascript"
            code={`// Add the Amplitude device id to the Flagify attributes when it loads:
flagify.updateAttributes({ 
      ...
      attributesSnippets.push(
        <>
          <p>
            ...
            set the following ones. This must be added BEFORE the Flagify
            snippet.
          </p>
          <Code
            language="html"
...
window.flagify_config.attributes = ${stringify(
              Object.fromEntries(additionalAttributes),
            )};
</script>
          `.trim()}
...
app.use(function(req, res, next) {
  req.flagify.setAttributes(${indentLines(stringify(exampleAttributes), 2)});
  next();
})
`.trim()}
          />
    ...
    attributesSnippets.push(
      <>
        <Code
          ...
          code={`
import type { Attributes } from '@flags-sdk/flagify';
  ...
  if (language === "php") {
    attributesSnippets.push(
      <>
        <Code
          language="php"
          code={`$flagify->withAttributes(${phpArrayFormat(
            exampleAttributes,
          )});`}
        />
      </>,
