...
import { paddedVersionString } from "@flagify/flagify";
...
export default function FlagifySetupCodeSnippet({
  language,
  version,
  apiKey,
  apiHost,
  ...
  if (language.match(/^nocode/)) {
    return (
      <>
        {eventTracker.startsWith("flagify") ? (
          <div>
            ...
            <DocLink docSection="managedWarehouseTracking">
              custom event tracking with Flagify Managed Warehouse
            </DocLink>
            <br />
            <br />
            <>
      ...
      properties: {
        ...
        product: product_id,
          ...
          <>
            ...
            the Flagify snippet above:
            <Code
              language="html"
              code={`
<script>
window.flagify_config = window.flagify_config || {};
window.flagify_config.trackingCallback = (experiment, result) => {
  ...
  if (language === "javascript") {
    return (
      <>
        ...
        />
        Create a Flagify instance. Read more about our{" "}
        ...
        />
        {eventTracker === "flagify" && (
          <>
            <br />
            If you want to use Flagify for experiments (and metrics), you
...
export default function MyApp() {
  useEffect(() => {
    // Load features asynchronously when the app renders${
      useInit
        ? `\n    flagify.init({ streaming: true });`
        : `\n    flagify.loadFeatures();`
    }
  }, []);
...
`.trim()}
        ...
        <a
          href="https://github.com/flagify/examples/tree/main/next-js"
          target="_blank"
          rel="noreferrer"
        >
          check out our sample app on GitHub
        </a>{" "}
        with examples of using Flagify with SSR, API routes, static pages,
        and more.
        {eventTracker === "flagify" && (
          <>
            <br />
...
gb.logEvent("Button Click", {
  ...
  if (language === "nodejs") {
    ...
    if (useMultiUser) {
      return (
        <>
          Create and initialize a Flagify client
          <Code
            language="javascript"
            code={`
const { FlagifyClient } = require("@flagify/flagify");
...
await client.init({ timeout: 1000 });
          `.trim()}
          />
          Use a middleware to create a Flagify instance that is scoped to the
...
app.use((req, res, next) => {
  ...
  req.flagify = client.createScopedInstance(userContext);
});
          `.trim()}
          />
        </>
...
setPolyfills({
  ...
  EventSource: require("eventsource")
        ...
        `.trim()}
        />
        Add a middleware before any routes that will use Flagify
        <Code
          language="javascript"
          code={`
const { Flagify } = require("@flagify/flagify");

app.use(function(req, res, next) {
  // Create a Flagify Context
  ...
  // Wait for features to load (will be cached in-memory for future requests)${
    useInit
      ? `\n  req.flagify.init({ timeout: 1000 })`
      : `\n  req.flagify.loadFeatures({ timeout: 1000 })`
  }
    .then(() => next())
})
`.trim()}
    ...
    return (
      <>
        ...
        <Code
          ...
          language="typescript"
          code={`
import { flagifyAdapter } from "@flags-sdk/flagify";
  `.trim()}
        />
        ...
        <table className="table w-auto gbtable table-sm table-bordered bg-light my-2">
          ...
          <tbody>
            ...
            <tr>
              <td className="px-3">
                <div>
                  <code>FLAGIFY_EDGE_CONNECTION_STRING</code>
            ...
            <tr>
              <td className="px-3">
                <code>FLAGIFY_EDGE_CONFIG_ITEM_KEY</code>
                <span className="text-muted small ml-2">(optional)</span>
              </td>
              <td className="px-3">
                <span className="text-muted">
...
flagifyAdapter.setTrackingCallback((experiment, result) => {
    ...
    return (
      <>
        ...
        <Code
          language="kotlin"
          code={`
import com.sdk.flagify.GBSDKBuilder
    ...
    return (
      <>
        ...
        <Code
          language="swift"
          code={`
var gb: FlagifySDK = FlagifyBuilder(
  ...
  if (language === "go") {
    return (
      <>
        Create Flagify client instance
        <Code
          language="go"
          code={`
package main
...
import (
	...
	"time"
	"encoding/json"
	gb "github.com/flagify/flagify-golang"
)
...
func main() {
  ...
  if (language === "ruby") {
    return (
      <>
        Get features from the Flagify API
        <Code
          language="ruby"
          code={`
require 'flagify'

# Fetch features from a Flagify instance
...
def on_experiment_viewed(experiment, result):
  ...
  print("Variation Id: " + result.key)
            `.trim()}
        />
        Create a Flagify instance and load features
        <Code
          language="python"
          code={`
...
GBContext context = GBContext.builder()
    ...
    .build();
Flagify growthBook = new Flagify(context);
            `.trim()
              : `
GBContext context = GBContext.builder()
    .featuresJson(featuresJson)
    ...
    return (
      <>
        ...
        <Code
          language="dart"
          code={`
final FlagifySDK gb = GBSDKBuilderApp(
  hostURL: '${apiHost}/',
  apiKey: "${apiKey}",
...
// Fetch feature flags from the Flagify API
...
var gb = new Flagify.Flagify(context);
    `.trim()}
        />
      </>
    );
...
features = Flagify.Config.features_from_config(features)
          `.trim()}
        />
      </>
    );
    ...
    return (
      <>
        ...
        <p>
          ...
          <a
            href="https://github.com/flagify/flagify-proxy/tree/main/packages/lib/edge-cloudflare/example"
            target="_blank"
            rel="noreferrer"
          >
        ...
        <Code
          language="javascript"
          code={`
import { handleRequest } from "@flagify/edge-cloudflare";
...
FLAGIFY_API_HOST=${JSON.stringify(apiHost)}
FLAGIFY_CLIENT_KEY=${JSON.stringify(apiKey)}${
            encryptionKey
              ? `\nFLAGIFY_DECRYPTION_KEY=${JSON.stringify(encryptionKey)}`
              : ""
          }
          `.trim()}
        />
        ...
        <p>
          Set up a <strong>Cloudflare KV</strong> store and use a Flagify{" "}
          <strong>SDK Webhook</strong> to keep feature and experiment values
          synced between Flagify and your Cloudflare Worker. This eliminates
          network requests from your edge to Flagify.
        </p>
    ...
    return (
      <>
        ...
        <p>
          ...
          to set up your project. Or have a look at our{" "}
          <a
            href="https://github.com/flagify/flagify-proxy/tree/main/packages/lib/edge-fastly/example"
            target="_blank"
            rel="noreferrer"
          >
            example implementation <FaExternalLinkAlt />
...
import { gbHandleRequest, getConfigEnvFromStore } from "@flagify/edge-fastly";
...
async function handleRequest(event) {
  ...
  const config = {
    // Name of Fastly backend pointing to your Flagify API Endpoint
  ...
  return await gbHandleRequest(event.request, env, config);
        ...
        <p>
          ...
          Flagify API by setting up backends (origins) for your Compute
...
FLAGIFY_CLIENT_KEY=${JSON.stringify(apiKey)}${
        ...
        <p>
          Set up a <strong>Fastly KV</strong> store and use a Flagify{" "}
          <strong>SDK Webhook</strong> to keep feature and experiment values
          synced between Flagify and your Fastly worker. This eliminates
          network requests from your edge to Flagify.
        </p>
    ...
    return (
      <>
        ...
        <Code
          language="javascript"
          code={`
import { handleRequest } from "@flagify/edge-lambda";
      ...
      encryptionKey
        ...
        <ul>
          <li>
            ...
            use a Flagify <strong>SDK Webhook</strong> to keep feature and
            experiment values synced between Flagify and your edge worker.
            This eliminates network requests from your edge to Flagify.
          </li>
          <li>
            Enable URL Redirect experiments on edge (off by default) by setting{" "}
            <code>{`RUN_URL_REDIRECT_EXPERIMENTS="everywhere"`}</code>
    ...
    return (
      <>
        ...
        <Code
          language="javascript"
          code={`
import { edgeApp, getConfig, defaultContext } from "@flagify/edge-utils";
...
FLAGIFY_CLIENT_KEY=${JSON.stringify(apiKey)}${
        ...
        <ul>
          <li>
            Set up an edge key-val store and use a Flagify{" "}
            <strong>SDK Webhook</strong> to keep feature and experiment values
            synced between Flagify and your edge worker. This eliminates
            network requests from your edge to Flagify.
          </li>
          <li>
            Enable URL Redirect experiments on edge (off by default) by setting{" "}
...
if (window.snowplow) {
  window.snowplow("trackSelfDescribingEvent", {
    event: {
      schema: "iglu:io.flagify/experiment_viewed/jsonschema/1-0-0",
      data: {
        experimentId: e.key,
        variationId: r.key,
...
const flagify = new Flagify({
  ...
  // Supports plugins, but with a different tracker
    ...
    jsCode = `
import { Flagify } from "@flagify/flagify";
import { autoAttributesPlugin } from "@flagify/flagify/plugins";
...
const flagify = new Flagify({
  ...
  // No plugins support
    ...
    jsCode = `import { Flagify } from "@flagify/flagify";
...
// Wait for features to be available${
      useInit
        ? `\nawait flagify.init({ streaming: true });`
