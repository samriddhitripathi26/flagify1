  ...
  if (language.match(/^nocode/)) {
    return (
      <Code
        ...
        code={`
<script>
const value = window._flagify?.getFeatureValue(
  ...
  if (language === "javascript") {
    return (
      <Code
        language="javascript"
        code={`
const value = flagify.getFeatureValue(
  ${JSON.stringify(featureId)},
  ${getDefaultValue(valueType)}
);
console.log(value);
  ...
  if (language === "react") {
    return (
      <Code
        language="tsx"
        code={`
import { useFeatureValue } from "@flagify/flagify-react";
  ...
  if (language === "nodejs") {
    return (
      <Code
        ...
        code={`
app.get("/", (req, res) => {
  const value = req.flagify.getFeatureValue(
    ${JSON.stringify(featureId)},
    ${getDefaultValue(valueType)}
  );
...
export const myFeatureFlag = flag<${typeString}>({
  ...
  adapter: flagifyAdapter.feature<${typeString}>(),
  ...
  if (language === "php") {
    return (
      <Code
        language="php"
        code={`
$value = $flagify->getValue(${JSON.stringify(featureId)}, ${getDefaultValue(
          valueType,
          "[]",
        )});
echo $value;
  ...
  if (language === "elixir") {
    return (
      <Code
        language="elixir"
        code={`
feature = Flagify.feature(context, ${JSON.stringify(featureId)})
