  ...
  const validateResponse = (result: TestQueryRow) => {
    ...
    if (!userEnteredHasNameCol) {
      ...
      // Only selected `experiment_name`, add warning
        ...
        throw new Error(
          "Missing variation_name column. Please add it to your SELECT clause to enable Flagify to populate names automatically or remove experiment_name.",
        );
      }
      // Only selected `variation_name`, add warning
      else if (returnedColumns.has("variation_name")) {
        throw new Error(
          "Missing experiment_name column. Please add it to your SELECT clause to enable Flagify to populate names automatically or remove variation_name.",
        );
      }
    } else {
