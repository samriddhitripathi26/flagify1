...
import { getFlagifyBuild, isCloud } from "@/services/env";
...
export default function UpgradeModal({
  ...
  const startProTrial = async function (name?: string, email?: string) {
    ...
    try {
      ...
      setError(
        `There was a server error: ${txt}. Please try again later, or contact us at sales@flagify.io`,
      );
    }
  };
    ...
    try {
      ...
      switch (txt) {
        case "active license exists":
          setError(
            "You already have an active license key. Contact us at sales@flagify.io for more information.",
          );
          break;
        case "expired license exists":
          setError(
            "Your license key has already expired. Please contact us at sales@flagify.io for more information.",
          );
          break;
        default:
          setError(
  ...
  const bullets: Partial<Record<CommercialFeature, string>> = {
    ...
    "cloud-proxy":
      "Use a self-hosted Flagify proxy in front of our Cloud CDN",
  ...
  const enterpriseCallout = (
    <p className="mb-0" style={{ color: "var(--color-text-mid)" }}>
      ...
      <a
        href="https://www.flagify.io/demo"
    ...
    return (
      ...
      >
        <Callout status="info" mr="5" mb="2">
          Your organization is already on Flagify&apos;s highest plan.
        </Callout>
      </Modal>
    );
  }
            ...
            <Callout status="info">
              ...
              {showEnterpriseTreatment ? (
                ...
                <span className="pl-1">
                  ...
                  Flagify integration. From there, you can upgrade your
                  Flagify subscription via the <b>Settings </b>tab.
                </span>
              )}
            </Callout>
          </div>
