...
export default function PublicExperimentPage(props: PublicExperimentPageProps) {
  ...
  return (
    <div className={`public pb-2 ${isBandit ? "bandit" : "experiment"}`}>
      <Head>
        <title>{experiment?.name ? `${experiment.name} | Flagify` : "Experiment not found | Flagify"}</title>
        <meta
          property="og:title"
          content={experiment?.name ? (`${isBandit ? "Bandit" : "Experiment"}: ${experiment?.name} | Flagify`) : "Experiment not found | Flagify"}
        />
        <meta
