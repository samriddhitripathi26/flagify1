...
export async function buildImportedData(
  ...
  // Create mapping from Statsig project name to Flagify project ID
  // Statsig uses project names (which they call "id") while Flagify uses internal IDs
  ...
  try {
    ...
    if (apiCall && existingAttributeSchema) {
      try {
        ...
        // This includes existing Flagify features and Statsig features being imported
            ...
            if (segmentImport.segment) {
              try {
                ...
                if (segmentImport.existingSavedGroup) {
                  // Prepare existing data (scrubbed and sorted)
                  // Note: existing data should already have Flagify project IDs,
                  ...
                  // Prepare transformed data (scrubbed and sorted)
                  // Transformed data has Statsig project names that need mapping to Flagify IDs
                  const transformedForDiff = transformPayloadForDiffDisplay(
                    transformed as Record<string, unknown>,
                    "segment",
                    projectNameToIdMap,
...
export async function runImport(options: RunImportOptions) {
  ...
  // Map to track StatSig segment names to Flagify saved group IDs
  const savedGroupIdMap = new Map<string, string>();
    ...
    data.segments?.forEach((segment, index) => {
      ...
      queue.add(async () => {
        ...
        try {
          ...
          // Transform Statsig segment to Flagify saved group using the
          ...
          // Map Statsig segment name to Flagify saved group ID
    ...
    if (
      ...
      queue.add(async () => {
        ...
        try {
          ...
          // Transform Statsig feature gate to Flagify feature
    ...
    if (
      ...
      queue.add(async () => {
        ...
        try {
          ...
          // Transform Statsig dynamic config to Flagify feature
          // Get available environments from the processed data
          const availableEnvironments =
            data.environments
              ?.map((e) => e.environment?.name || e.key)
    ...
    if (
      ...
      queue.add(async () => {
        ...
        try {
          ...
          // Transform Statsig experiment to Flagify experiment
