import { ConditionInterface } from "@flagify/flagify-react";
...
/**
 * Transform Statsig conditions to Flagify format
 */
export function transformStatsigConditionsToGB(
  ...
  // Convert targeting conditions to Flagify format
...
/**
 * Transform targeting conditions to Flagify condition string
 */
function transformTargetingConditions(
  conditions: StatsigCondition[],
  skipAttributeMapping: boolean = false,
): string {
  // Map Statsig operators to Flagify operators
  ...
  conditions.forEach((condition) => {
    ...
    if (operator === "str_contains_any") {
      ...
      // Flagify allows multiple operators on the same attribute (e.g. os: { $in: [...], $nini: [...] })
